...
@Service("com.ts.service.FoodService")
...
public class FoodService {
	...
	public int callFoodApi() throws Exception {
		int complete = -1;
		...
		complete = foodCreate(jsonArray, openApiVo);
		return complete;
	}
	 ...
	 * 부산 맛집데이터를 생성합니다.
	 */
	public int foodCreate(JSONArray jsonArray, OpenApiVo openApiVo) {
		int complete = -1;

		openApiMapper.food_delete(openApiVo);
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			result = openApiMapper.food_save(params);
			if (result > -1) {
	 ...
	 * 부산 맛집데이터를 전체 삭제합니다.
	 */
	public int foodDelete(OpenApiVo openApiVo) {
		int complete = -1;
		complete = openApiMapper.food_delete(openApiVo);
		return complete;
	}
}
